...
The publish step re-applies the exact validated patch, verifies its SHA-256 and tree digests, checks that only the validated file list is staged, and requires the original open PR and source branch to still point to the collected head SHA and base branch immediately before commit. It re-reads issue markers and the deterministic remediation branch immediately before commit. An existing trusted marker returns `NOOP`; an existing branch is claimed or recovered only through an atomic ref creation plus exact parent/tree verification, and the non-force push cannot overwrite a competing result. A branch left by an interrupted run remains recoverable and does not suppress a later `READY` finding. The final PR creation is idempotent: an existing open PR is reused only when it has the exact remediation branch as head, the original source branch as base, and draft state; if creation races, the workflow re-queries and validates that same record. It creates:
