...
4. **Publish** — write access only after validation. Recreate the validated worktree, re-fetch the original PR and source branch identity at the exact collected SHA, commit only the verified files with hooks disabled, verify the staged file list and clean worktree/index boundary, re-check the trusted marker history and deterministic remediation branch, atomically claim the deterministic branch at the source SHA, recover a source-SHA placeholder with a fast-forward-only push, verify the exact parent and resulting tree for a completed branch, and create a draft PR against the original source branch. If a concurrent run already created the deterministic branch, the workflow classifies the remote tip before idempotently creating or reusing its open draft PR. The body contains bounded evidence metadata, tests, rollback instructions, and an explicit no-approval statement.
...
Each event uses its own non-canceling concurrency group keyed by the durable review or check-run identifier. Events without the required identifier are rejected by collection; a shared pull-request group would use GitHub Actions' single pending slot as an unsafe event queue and could discard findings during a burst. Publication first atomically claims the deterministic branch at the source SHA. A losing or restarted run advances that explicit placeholder with a fast-forward-only push, or verifies the completed branch's parent/tree before recovering the draft PR; a non-force push cannot overwrite a competing result. Existing open PRs are accepted only when their head, base, and draft state match; a create race is re-queried before success.
...
4. The trusted `semantic-syntax` gate parses changed JavaScript/Rust files without executing source-controlled code. It skips files absent after a valid deletion and invokes `rustfmt --emit stdout` for Rust so formatting is not required by this gate.
...
The publish step re-applies the exact validated patch, verifies its SHA-256 and tree digests, checks that only the validated file list is staged, and requires the original open PR and source branch to still point to the collected head SHA and base branch immediately before commit. It re-reads issue markers and the deterministic remediation branch immediately before commit. An existing trusted marker returns `NOOP`; an existing branch is claimed or classified after an atomic ref creation: a source-SHA placeholder is advanced only by fast-forward, while a completed branch must have the exact parent/tree. The non-force push cannot overwrite a competing result. A branch left by an interrupted run remains recoverable and does not suppress a later `READY` finding. The final PR creation is idempotent: an existing open PR is reused only when it has the exact remediation branch as head, the original source branch as base, and draft state; if creation races, the workflow re-queries and validates that same record. It creates:
...
- Stale source SHA or changed source branch between jobs: discard the artifact and require a fresh review event. A source-SHA placeholder left by an interrupted publication may be advanced only by fast-forward; a completed remediation branch is reused only when its parent and tree match the validated publication.
...
- identity/idempotency: exact SHA/tree/repository binding, live publication revalidation, staged file-list binding, publication-time marker/branch claim, source-SHA placeholder recovery, exact parent/tree recovery, and one branch/PR per fingerprint;
