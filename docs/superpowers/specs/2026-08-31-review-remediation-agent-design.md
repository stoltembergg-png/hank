...
Each event uses its own non-canceling concurrency group keyed by `github.run_id`; a shared pull-request group would use GitHub Actions' single pending slot as an unsafe event queue and could discard findings during a burst. Publication uses the deterministic branch as a compare-and-swap claim: a non-force push wins the claim, and a losing or restarted run verifies the branch parent/tree before recovering the draft PR. Existing open PRs are accepted only when their head, base, and draft state match; a create race is re-queried before success.
