...
Download the patch artifact, check out the exact PR head into a separate target directory with credentials disabled, run the trusted helper against that directory, and execute deterministic patch checks plus trusted syntax parsing for changed JavaScript/Rust files without `GITHUB_TOKEN` or MiMo credentials in the environment. The parser accepts valid deletions and uses `rustfmt --emit stdout` for Rust without requiring formatting. The publish job repeats patch/digest/tree validation, re-fetches the original PR and source branch identity immediately before publication, re-checks the trusted marker history and deterministic branch claim, verifies the staged file list and clean worktree/index boundary, uses `git -c core.hooksPath=/dev/null`, pushes only the generated branch without force, and calls `gh pr create --draft` with a bounded body containing the evidence marker, source SHA, tests, and rollback. If a concurrent run already created the branch, it classifies a source-SHA placeholder for fast-forward recovery or verifies the exact source parent and resulting tree before reusing a completed branch, then checks for an existing open PR before creating one. It never runs a script from `target/`; the generated draft's normal required CI is authoritative for repository checks. Each event uses its own non-canceling concurrency group keyed by the durable review/check-run identifier; events without that identifier are rejected during collection.
