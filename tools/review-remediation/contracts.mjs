...
const SECRET_ASSIGNMENT_NAME = '(?:api[_-]?(?:key|token)|access[_-]?token|auth|authorization|auth[_-]?token|client[_-]?secret|refresh[_-]?token|password|passwd|private[_-]?key|secret|token)';
