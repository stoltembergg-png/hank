...
function readStatus() {
  const result = spawnSync('git', ['status', '--porcelain=v1', '-z'], { cwd: root, encoding: 'buffer' });
...
const initialPaths = readStatus();
const nonSidecar = (path) => path && !path.startsWith('.spec/verification/') && !path.startsWith('security/reports/');
...
if (commit.status !== 0 || tree.status !== 0) throw new Error('could not resolve exact source identity');
process.stdout.write(`# sourceCommit=${commit.stdout.trim()}\n# sourceTree=${tree.stdout.trim()}\n${tap}`);
