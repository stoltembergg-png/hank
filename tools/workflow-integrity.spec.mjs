...
test('review remediation keeps MiMo and publication boundaries explicit', () => {
  ...
  assert.match(text, /group:\s*review-remediation-\$\{\{\s*github\.repository\s*\}\}-\$\{\{\s*github\.event\.pull_request\.number\s*\|\|\s*github\.event\.check_run\.pull_requests\[0\]\.number\s*\|\|\s*github\.event\.check_run\.id\s*\}\}-\$\{\{\s*github\.event_name\s*\}\}-\$\{\{\s*github\.event\.review\.user\.login\s*\|\|\s*github\.event\.check_run\.name\s*\|\|\s*'event'\s*\}\}/);
